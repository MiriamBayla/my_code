...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if i < 0 or i >= self.height or j < 0 or j >= self.width:
                    continue
                # If the given cell in unown to be mine or safe and wasn't checked yet - add it to neighbors
                neighbor = (i, j)
        ...
        while changed:
            ...
            # Implications of mines as mines and safes as safes
            for cell in mines:
            ...
            # Updating sentences by subtracting similar things
            new_sentences = []
            for s1 in self.knowledge:
                for s2 in self.knowledge:
                    # Avoid comparing a cell to itself
                    if s1 == s2:
                        continue
                    # Check in a sentence is a subset of the other
                    if s1.cells.issubset(s2.cells):
                        new_cells = s2.cells - s1.cells
                        new_count = s2.count - s1.count
                        new_sentence = Sentence(new_cells, new_count)

                        # Avoid duplication and empty sentences
